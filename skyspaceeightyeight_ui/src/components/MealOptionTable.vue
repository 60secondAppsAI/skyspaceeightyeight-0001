        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Meal Options</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalMealOptions = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMealOptions">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MealOption</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MealOptionId" type="text" placeholder="Enter MealOptionId" v-model="mealOptionToAdd.mealOptionId"></base-input>
  <base-input label="Description" type="text" placeholder="Enter Description" v-model="mealOptionToAdd.description"></base-input>
  <base-input label="DietaryRestrictions" type="text" placeholder="Enter DietaryRestrictions" v-model="mealOptionToAdd.dietaryRestrictions"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="mealOptions" :row-key="record => record.MealOptionId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MealOptionPictureView :mealOptions="mealOptions" />
          </div>
...
import MealOptionPictureView from './MealOptionPictureView.vue';


const mealOptionsColumns = [
  "mealOptionId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    MealOptionPictureView
  },

  data() {
    return {
      modalMealOptions: false,
        isTableView: true,

      columns: [
        {
          title: 'Meal Option Id',
		dataIndex: 'mealOptionId',
          visible: true,
          scopedSlots: { customRender: 'mealOptionId' },
          sorter: true
          //sorter: (a, b) => a.mealOptionId - b.mealOptionId,
          //sorter: (a, b) => a.mealOptionId.localeCompare(b.mealOptionId),
        },
        {
          title: 'Description',
		...
		dataIndex: 'dietaryRestrictions',
          visible: true,
          scopedSlots: { customRender: 'dietaryRestrictions' },
          sorter: true
          //sorter: (a, b) => a.dietaryRestrictions - b.dietaryRestrictions,
          //sorter: (a, b) => a.dietaryRestrictions.localeCompare(b.dietaryRestrictions),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} mealOptions`,
      },
      ...
      sortBy: 'mealOptionId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMealOptionsTable() {
      ...
      let mealOptionsTableData = [];
      for (let i = 0; i < this.mealOptions.length; i++) {
        mealOptionsTableData.push({
          id: i,
          mealOptionId: this.mealOptions[i].mealOptionId,
          year: this.mealOptions[i].year,
          date: this.mealOptions[i].date,
          competitionId: this.mealOptions[i].competitionId,
          mealOptionId: this.mealOptions[i].mealOptionId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-meal-options',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMealOptionsChanged', this.searchQuery);
		//this.renderMealOptionsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMealOptions = false;

      const currentDate = new Date().getTime();
      this.mealOptionToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.mealOptionToAdd);
      console.log(jsonData);
      
      const res = await MealOptionService.addMealOption(jsonData);
