        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Flight Schedules</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalFlightSchedules = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFlightSchedules">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add FlightSchedule</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="FlightScheduleId" type="text" placeholder="Enter FlightScheduleId" v-model="flightScheduleToAdd.flightScheduleId"></base-input>
  <base-input label="DepartureDate" type="text" placeholder="Enter DepartureDate" v-model="flightScheduleToAdd.departureDate"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="flightSchedules" :row-key="record => record.FlightScheduleId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FlightSchedulePictureView :flightSchedules="flightSchedules" />
          </div>
...
import FlightSchedulePictureView from './FlightSchedulePictureView.vue';


const flightSchedulesColumns = [
  "flightScheduleId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    FlightSchedulePictureView
  },

  data() {
    return {
      modalFlightSchedules: false,
        isTableView: true,

      columns: [
        {
          title: 'Flight Schedule Id',
		dataIndex: 'flightScheduleId',
          visible: true,
          scopedSlots: { customRender: 'flightScheduleId' },
          sorter: true
          //sorter: (a, b) => a.flightScheduleId - b.flightScheduleId,
          //sorter: (a, b) => a.flightScheduleId.localeCompare(b.flightScheduleId),
        },
        {
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} flightSchedules`,
      },
      ...
      sortBy: 'flightScheduleId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFlightSchedulesTable() {
      ...
      let flightSchedulesTableData = [];
      for (let i = 0; i < this.flightSchedules.length; i++) {
        flightSchedulesTableData.push({
          id: i,
          flightScheduleId: this.flightSchedules[i].flightScheduleId,
          year: this.flightSchedules[i].year,
          date: this.flightSchedules[i].date,
          competitionId: this.flightSchedules[i].competitionId,
          flightScheduleId: this.flightSchedules[i].flightScheduleId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-flight-schedules',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFlightSchedulesChanged', this.searchQuery);
		//this.renderFlightSchedulesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalFlightSchedules = false;

      const currentDate = new Date().getTime();
      this.flightScheduleToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.flightScheduleToAdd);
      console.log(jsonData);
      
      const res = await FlightScheduleService.addFlightSchedule(jsonData);
