        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMaintenanceRecords = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMaintenanceRecords">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MaintenanceRecord</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MaintenanceRecordId" type="text" placeholder="Enter MaintenanceRecordId" v-model="maintenanceRecordToAdd.maintenanceRecordId"></base-input>
  <base-input label="Date" type="text" placeholder="Enter Date" v-model="maintenanceRecordToAdd.date"></base-input>
  <base-input label="Details" type="text" placeholder="Enter Details" v-model="maintenanceRecordToAdd.details"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="maintenanceRecords" :row-key="record => record.MaintenanceRecordId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MaintenanceRecordPictureView :maintenanceRecords="maintenanceRecords" />
          </div>
...
import MaintenanceRecordPictureView from './MaintenanceRecordPictureView.vue';


const maintenanceRecordsColumns = [
  "maintenanceRecordId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    MaintenanceRecordPictureView
  },

  data() {
    return {
      modalMaintenanceRecords: false,
        isTableView: true,

      columns: [
        {
          title: 'Maintenance Record Id',
		dataIndex: 'maintenanceRecordId',
          visible: true,
          scopedSlots: { customRender: 'maintenanceRecordId' },
          sorter: true
          //sorter: (a, b) => a.maintenanceRecordId - b.maintenanceRecordId,
          //sorter: (a, b) => a.maintenanceRecordId.localeCompare(b.maintenanceRecordId),
        },
        {
          title: 'Date',
		dataIndex: 'date',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} maintenanceRecords`,
      },

      maintenanceRecords: [],
      maintenanceRecordToAdd: {},
      ...
      sortBy: 'maintenanceRecordId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMaintenanceRecordsTable() {
      ...
      let maintenanceRecordsTableData = [];
      for (let i = 0; i < this.maintenanceRecords.length; i++) {
        maintenanceRecordsTableData.push({
          id: i,
          maintenanceRecordId: this.maintenanceRecords[i].maintenanceRecordId,
          year: this.maintenanceRecords[i].year,
          date: this.maintenanceRecords[i].date,
          competitionId: this.maintenanceRecords[i].competitionId,
          maintenanceRecordId: this.maintenanceRecords[i].maintenanceRecordId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-maintenance-records',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMaintenanceRecordsChanged', this.searchQuery);
		//this.renderMaintenanceRecordsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMaintenanceRecords = false;

      const currentDate = new Date().getTime();
      this.maintenanceRecordToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.maintenanceRecordToAdd);
      console.log(jsonData);
      
      const res = await MaintenanceRecordService.addMaintenanceRecord(jsonData);
