        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Crew Assignments</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalCrewAssignments = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCrewAssignments">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add CrewAssignment</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="CrewAssignmentId" type="text" placeholder="Enter CrewAssignmentId" v-model="crewAssignmentToAdd.crewAssignmentId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="crewAssignments" :row-key="record => record.CrewAssignmentId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CrewAssignmentPictureView :crewAssignments="crewAssignments" />
          </div>
...
import CrewAssignmentPictureView from './CrewAssignmentPictureView.vue';


const crewAssignmentsColumns = [
  "crewAssignmentId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    CrewAssignmentPictureView
  },

  data() {
    return {
      modalCrewAssignments: false,
        isTableView: true,

      columns: [
        {
          title: 'Crew Assignment Id',
		dataIndex: 'crewAssignmentId',
          visible: true,
          scopedSlots: { customRender: 'crewAssignmentId' },
          sorter: true
          //sorter: (a, b) => a.crewAssignmentId - b.crewAssignmentId,
          //sorter: (a, b) => a.crewAssignmentId.localeCompare(b.crewAssignmentId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} crewAssignments`,
      },
      ...
      sortBy: 'crewAssignmentId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCrewAssignmentsTable() {
      ...
      let crewAssignmentsTableData = [];
      for (let i = 0; i < this.crewAssignments.length; i++) {
        crewAssignmentsTableData.push({
          id: i,
          crewAssignmentId: this.crewAssignments[i].crewAssignmentId,
          year: this.crewAssignments[i].year,
          date: this.crewAssignments[i].date,
          competitionId: this.crewAssignments[i].competitionId,
          crewAssignmentId: this.crewAssignments[i].crewAssignmentId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-crew-assignments',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCrewAssignmentsChanged', this.searchQuery);
		//this.renderCrewAssignmentsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalCrewAssignments = false;

      const currentDate = new Date().getTime();
      this.crewAssignmentToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.crewAssignmentToAdd);
      console.log(jsonData);
      
      const res = await CrewAssignmentService.addCrewAssignment(jsonData);
